...
def calculate_model():    
    ...
    # csp_feature_index = np.array([0,-1])

    # pairs of CSP features selected in mutual info
    k=6

    trianingDataFolder = './training_data_curctrl/'

    with open('task_markers.json', 'r') as file:
        markers = json.load(file)
        

    label_left_int = markers['curctrl_up'][0]
    label_right_int = markers['curctrl_down'][0]
    ...
    signal_win_start = 0 #second
    signal_win_end = 2.2 #second
    cut_peroid_sec = signal_win_end-1
    ...
    for cl in cl_lab:
        ...
        # tmp3 = tmp[:50,:,int(fs*cut_peroid_sec):]
        tmp3 = tmp[:,:,int(fs*cut_peroid_sec):]
        # print(tmp3.shape)
        # tmp3 = tmp[:,:,:-1] 
        #epochs will give the data 1 more sample, for example, fs is 128, 1 second window, then there will be 129 samples
        trials_raw[cl] = tmp3.transpose((1, 2, 0)).copy()
        tmp2 = tmp3.reshape(tmp3.shape[0],-1)
        trials_max_value[cl] = np.max(tmp2,axis=1)

    # # deleted bad trials
    bad_trial_thr = 0.00012
    ...
    # plt.plot([0,max(trials_max_value['right'].shape[0],trials_max_value['left'].shape[0])],[bad_trial_thr,bad_trial_thr],':')
    ...
    # 7. 28-32Hz
    band_interval = 4
    filter_band = np.arange(4, 24, step = int(band_interval/2))
    ...
    for band_key, trials in trials_all.items():         
        ...
        # nsamples_win = trials['left'].shape[1]
        ...
        trials_logvar = {'left': np.log(np.var((trials_csp_f['left']), axis=1)),'right': np.log(np.var((trials_csp_f['right']), axis=1))}
    ...
    y = np.concatenate((np.ones(trial_num['left']),np.zeros(trial_num['right'])),axis = 0)
    ...
    y = np.concatenate((np.ones(trial_num_train['left']),np.zeros(trial_num_train['right'])),axis = 0)
    ...
    std = StandardScaler()
    X_train_std = std.fit_transform(X_train_mutual_info)
    X_test_std = std.transform(X_test_mutu_info)
    ...
    # y_test = np.concatenate((np.ones(trial_num_test['left']),np.zeros(trial_num_test['right'])),axis = 0)
    ...
    optimal_params.fit(X_train_std, y)
    ...
    tmp_X_test_std = X_test_std[0][:].reshape(-1,1).T
    r_proba_1=clf.predict_proba(tmp_X_test_std)

    # r=clf.predict(X_test_std)
    r_proba=clf.predict_proba(X_test_std)
    r=clf.predict(X_test_std)
    ...
    conf = np.array([
        [(r[0:trial_num_test['left']] == 1).sum(), (r[trial_num_test['left']:] == 1).sum()],
        [(r[0:trial_num_test['left']] == 0).sum(), (r[trial_num_test['left']:] == 0).sum()],
    ])
